...
export default class CreateScratchOrgImpl {
  ...
  public constructor(private working_directory:string,private config_file_path: string, private devhub: string,private alias: string) {}
  ...
  public async buildExecCommand(): Promise<string> {
    let command = `npx sfdx force:org:create -v ${this.devhub} -s -f ${this.config_file_path} --json -a ${this.alias} -d 1`;
    return command;
  }
